...
export async function initYandex(){
  try{
    ...
    if(!loaded||!window.YaGames){
      console.info('[Yandex SDK] unavailable, using local mode');
...
export function showRewardedAd({onRewarded,onOpen,onClose,onError}={}){
  if(!ysdk?.adv?.showRewardedVideo){onError?.(new Error('Rewarded video is unavailable outside Yandex Games'));return false}
  try{ysdk.adv.showRewardedVideo({callbacks:{onOpen,onRewarded,onClose,onError}});return true}
  catch(error){onError?.(error);return false}
}
export function showInterstitialAd({onOpen,onClose,onError}={}){
  if(!ysdk?.adv?.showFullscreenAdv){onClose?.();return false}
  try{ysdk.adv.showFullscreenAdv({callbacks:{onOpen,onClose,onError}});return true}
  catch(error){onError?.(error);onClose?.();return false}
}
