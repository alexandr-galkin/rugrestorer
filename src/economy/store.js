...
export function hasItem(id){const i=getInventory();return id==='cleaner'?(i.consumables.cleaner||0)>0:!!i.tools[id]}
...
export function consumeCleaner(){const i=getInventory();if((i.consumables.cleaner||0)<=0)return false;i.consumables.cleaner--;write('rugrestorer.inventory',i);return true}
export function completeDirt(level){const i=getInventory();i.completedDirt=Math.max(i.completedDirt,level);i.unlockedDirt=Math.min(LEVEL_COUNT,Math.max(i.unlockedDirt,level+1));write('rugrestorer.inventory',i);return i}
